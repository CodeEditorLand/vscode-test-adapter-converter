...
export class TestConverter implements vscode.Disposable {
	...
	constructor(private readonly adapter: TestAdapter) {
					...
					case "started":
						if (!this.doneDiscovery) {
							vscode.window.withProgress(
								{
									title: `An adapter is discovering tests`,
		...
		if (adapter.retire) {
			this.disposables.push(
				adapter.retire((evt) => {
					if (evt.tests) {
						const items = evt.tests
							.map((test) => this.itemsById.get(test))
							.filter((item) => !!item);
	...
	public async run(
		...
		const listener = this.adapter.testStates((evt) => {
			...
			this.tasksByRunId.set(evt.testRunId ?? "", {
				task: run,
				announcedTests: new Set(),
			});
	...
	private syncItemChildren(
		...
		collection.replace(
			unique(children, (c) => c.id).map((item) =>
				this.createTest(item, defaultUri),
			),
		);
	}
	...
	private onTestSuiteEvent(evt: TestSuiteEvent) {
		...
		const suiteId =
			typeof evt.suite === "string" ? evt.suite : evt.suite.id;

		if (evt.state === "running") {
			if (
				!this.itemsById.has(suiteId) &&
				typeof evt.suite === "object" &&
				runningSuite
			) {
			...
			if (this.itemsById.has(suiteId)) {
				this.runningSuiteByRunId.set(
					runId,
					this.itemsById.get(suiteId)!,
				);
			}
		} else {
	 ...
	 */
	private onTestEvent(
		{ task, announcedTests }: IRunningTaskData,
		evt: TestEvent,
	) {
		...
		switch (evt.state) {
			...
			case "failed":
				...
				for (const decoration of evt.decorations ?? []) {
					...
					const uri = decoration.file
						? fileToUri(decoration.file)
						: vscodeTest.uri;
		...
		if (
			evt.message &&
			((evt.state !== "errored" && evt.state !== "failed") ||
				!vscodeTest.uri)
		) {
